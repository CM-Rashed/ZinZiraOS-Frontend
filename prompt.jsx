...
import { 
  ...
  Calendar, Clock, CheckCircle2, XCircle, AlertCircle, UserCheck, CheckSquare, Mail
} from "lucide-react";
...
const STAFF_OTP_BASE_URL = `${SERVER_BASE_URL}/api/staff`;
...
export default function ManageStaff() {
  ...
  const [otpStep, setOtpStep] = useState(1); // 1 = Details Input, 2 = Verify OTP
  ...
  // Step 1: Request OTP or Direct Edit Submit
  const handleSubmitDetails = async (e) => {
    ...
    if (editingStaff) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/${editingStaff.id}`, {
          method: "POST",
    ...
    // Step 1 Registration: Request OTP
    ...
    try {
      ...
      const result = await res.json();
      if (!res.ok) throw new Error(result.message || "Failed to request OTP code.");

      setSuccessMsg(`OTP sent to ${formData.email}. Please check inbox.`);
      setOtpStep(2);
    } catch (err) {
      setErrorMsg(err.message);
    } finally {
          ...
          {filteredStaff.map((staff) => (
            ...
            >
              <div>
                ...
                {staff.email && (
                  <div className={styles.msStaffId} style={{ marginTop: "0.25rem" }}>
                    <Mail size={12} />
                    {staff.email}
                  </div>
                )}
      ...
      {isFormModalOpen && (
        <div className={styles.msModalOverlay}>
          <div className={styles.msModalContent}>
            ...
            {successMsg && <div style={{ background: "rgba(16, 185, 129, 0.2)", border: "1px solid #10b981", color: "#10b981", padding: "0.75rem", borderRadius: "0.5rem", marginBottom: "1rem", fontSize: "0.875rem" }}>{successMsg}</div>}

            {/* Step 1: Input Registration Details */}
            {otpStep === 1 && (
              <form onSubmit={handleSubmitDetails} className={styles.msForm}>
                <div className={styles.msPhotoUploadSection}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: "0.6875rem", color: "#64748b" }}>JPG, PNG, WEBP up to 2MB</p>
                  </div>
                </div>
                ...
                <div className={styles.msFormGrid}>
                  ...
                  <div className={styles.msInputGroup}>
                    <label className={styles.msLabel}>Email Address</label>
                    <input
                      type="email"
                      required
                  ...
                  {!editingStaff && (
                    <>
                      <div className={styles.msInputGroup}>
                        <label className={styles.msLabel}>Password</label>
                        <input
                          type="password"
                          required
                      ...
                      <div className={styles.msInputGroup}>
                        <label className={styles.msLabel}>Confirm Password</label>
                        <input
                          type="password"
                          required
            ...
            {/* Step 2: Input Verification OTP Code */}
            {otpStep === 2 && !editingStaff && (
              <form onSubmit={handleVerifyOtp} className={styles.msForm}>
                <p style={{ color: "#94a3b8", fontSize: "0.875rem", marginBottom: "1.5rem" }}>
                  A 6-digit verification code has been dispatched to <strong style={{ color: "#38bdf8" }}>{formData.email}</strong>. Please enter it below to confirm registration.
                </p>

                <div className={styles.msInputGroup}>
                  <label className={styles.msLabel}>6-Digit OTP Code</label>
                  <input
                    type="text"
                    required
                    maxLength={6}
                    ...
                    className={styles.msInput}
                    placeholder="123456"
                    style={{ textAlign: "center", letterSpacing: "8px", fontSize: "1.25rem", fontFamily: "monospace" }}
                  />
                </div>

                <div className={styles.msModalFooter} style={{ marginTop: "2rem" }}>
                  <button type="button" onClick={() => setOtpStep(1)} className={styles.msBtnCancel}>
                    Back
                  </button>
                  <button type="submit" disabled={submitting || otpCode.length !== 6} className={styles.msBtnPrimary}>
                    {submitting ? "Verifying..." : "Confirm & Register Staff"}
                  </button>
                </div>
              </form>
            )}
          ...
          <div className={styles.msModalContent} style={{ maxWidth: "700px" }}>
            ...
            {activeTab === "profile" && (
              <div className={styles.msFormGrid} style={{ padding: "0.5rem" }}>
                ...
                <div className={styles.msInputGroup}>
                  <label className={styles.msLabel}>Email</label>
                  <p style={{ color: "#f8fafc", fontSize: "1rem" }}>{selectedStaff.email || "N/A"}</p>
                </div>
